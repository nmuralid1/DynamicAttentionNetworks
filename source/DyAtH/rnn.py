...
class DecoderHierAttn(nn.Module):
	def __init__(self, input_size,hidden_size,num_layers,output_size,rnnobject,encoder_sequence_length,decoder_sequence_length,dropoutpercentage,logger):
	...
	def forward(self, input, hidden_states,hidden_prev,contextvector,hierattnmethod='mean',cell_state=None):
		...
		if self.rnnobject=="LSTM":
			...
			self.logger.debug("Attention Energy Size = {} Attention Energies = {}".format(self.attn_energies(hidden_t[-1,:,:].unsqueeze(dim=0)).size(), self.attn_energies(hidden_t[-1,:,:].unsqueeze(dim=0)).data.cpu().numpy()[0,0:2,:30]))
			self.logger.debug("Attn Energy Sample = {}".format(self.attn_energies(hidden_t[-1,:,:].unsqueeze(dim=0)).data.cpu().numpy()[0,0,:].ravel().tolist()))
