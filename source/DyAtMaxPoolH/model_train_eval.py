...
def _decoder_sequence_iter(input_tensor,target_tensor,decoder,encoder_hiddenstate,SOS_TOKEN,teacher_forcing_ratio,prev_hidden_states,criterion,logger,sliding_attention,hierattnmethod,encoder_cellstate=None,test=False):
    ...
    encoder_hidden_states=prev_hidden_states#torch.mean(prev_hidden_states,dim=1).unsqueeze(1)  #Instantiated either to None or to be a mean of the encoder hidden states. (batchsize , 1, hiddensize)
    ...
    for di in range(sequence_length):
        ...
        if decoder.rnnobject=="LSTM":
            decoder_output, (decoder_hidden,decoder_cellstate),attentional_hidden_state,attention_vector,hierarchical_attentional_hidden_state = decoder(decoder_input,prev_hidden_states[:, -(decoder.sliding_windowsize):, :],
                                                                                                                                                         decoder_hidden,contextvector=encoder_hidden_states,
                                                                                                                                                         hierattnmethod=hierattnmethod,cell_state=decoder_cellstate)
        else:
            decoder_output, contextvector, decoder_hidden,attentional_hidden_state,attention_vector,hierarchical_attentional_hidden_state = decoder(decoder_input,prev_hidden_states[:, -(decoder.sliding_windowsize):, :],
                                                                                                                                                    decoder_hidden,contextvector=encoder_hidden_states,
                                                                                                                                                    hierattnmethod=hierattnmethod)
        ...
        if sliding_attention==True:
            #SLIDING ATTENTION
            prev_hidden_states=torch.cat((prev_hidden_states[:,-(decoder.sliding_windowsize - 1):,:],decoder_hidden[-1,:,:].unsqueeze(dim=0).permute(1,0,2)),dim=1) #for now just use decoder hidden state we can also use attentional_hidden state if required later. (batch_size , seq_len, hidden_size) dim of prev_hidden_states.
...
def train(inputbatch, targetbatch, encoder, decoder, encoder_optimizer, decoder_optimizer,num_layers,criterion,teacher_forcing_ratio,SOS_TOKEN,logger,sliding_attention,hierattnmethod):
    logger.debug("inside train(), inputbatch.size() = {}, targetbatch.size() = {}, teacher_forcing_ratio = {}, SOS_TOKEN = {}".format(inputbatch.size(),targetbatch.size(),teacher_forcing_ratio,SOS_TOKEN))
...
def testNew(inputbatch, targetbatch, encoder, decoder,num_layers,criterion,SOS_TOKEN,teacher_forcing_ratio,logger,sliding_attention,hierattnmethod,istest=False):
    ...
    logger.debug("inside test(), inputbatch.size() = {}, targetbatch.size() = {}, SOS_TOKEN = {}".format(inputbatch.size(),targetbatch.size(),SOS_TOKEN))
    #Run Encoder
    if encoder.rnnobject=="LSTM":
...
def testIters(encoder, decoder, testdata, numbatches, num_layers,SOS_TOKEN,teacher_forcing_ratio,logger,sliding_attention=False,hierattnmethod="mean",targets=None):
    ...
    for idx,batch in enumerate(testdata.chunk(numbatches,dim=0)):
        ...
        hiddenstsimmat.append(hiddenstatesimilaritymatrix.cpu().detach())
        mean_squared_error.extend(mean_squared_error_batch)
        predictions.append(predictions_batch.cpu().detach())
        attention_weights.append(attention_weights_batch.cpu().detach())
        target_data.append(target_tensor.cpu().detach())
        ...
        del input_tensor, target_tensor, predictions_batch, mean_squared_error_batch, attention_weights_batch, hiddenstatesimilaritymatrix,loss
...
def trainIters(encoder, decoder, inputdata,testdata, numbatches, num_layers,teacher_forcing_ratio,NUM_EPOCHS,SOS_TOKEN,logger,sliding_attention=False,hierattnmethod='mean',print_every=2, plot_every=2, learning_rate=0.001,early_stopping_threshold=35,validation_tolerance=1e-3):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if patience>=early_stopping_threshold:
            break
        elif len(val_mse)>0 and ((np.abs(val_mse[-1] - epoch_val_loss)/val_mse[-1]) < validation_tolerance):
            patience+=1
